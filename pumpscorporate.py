...
def gerar_diagrama_rede(sistema, vazao_total, distribuicao_vazao, fluido, materiais_combinados, fluidos_combinados):
    ...
    # Trechos ANTES da divisão
    for i, trecho in enumerate(sistema['antes']):
        proximo_no = f"no_antes_{i+1}"
        # MUDANÇA: Captura todo o dicionário de perdas
        ...
        perda_trecho = perdas_info['principal'] + perdas_info['localizada']
        # MUDANÇA: Adiciona a perda de carga ao label
        label = f"Trecho Antes {i+1}\\n{vazao_total:.1f} m³/h\\n{velocidade:.2f} m/s\\nPerda: {perda_trecho:.2f} m"
    ...
    if len(sistema['paralelo']) >= 2 and distribuicao_vazao:
        ...
        for nome_ramal, trechos_ramal in sistema['paralelo'].items():
            ...
            for i, trecho in enumerate(trechos_ramal):
                # MUDANÇA: Captura todo o dicionário de perdas para o trecho do ramal
                ...
                perda_trecho_ramal = perdas_info_ramal['principal'] + perdas_info_ramal['localizada']
                # MUDANÇA: Adiciona a perda de carga ao label do ramal
                label_ramal = f"{nome_ramal} (T{i+1})\\n{vazao_ramal:.1f} m³/h\\n{velocidade:.2f} m/s\\nPerda: {perda_trecho_ramal:.2f} m"
